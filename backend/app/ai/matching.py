...
def _bucket(score: float, cfg: MatchConfig) -> str:
    ...
    if score >= cfg.moderately_qualified:
        return "Moderately Qualified"
    if score >= cfg.qualified:
        return "Qualified"
    if score >= cfg.for_review:
        return "For Review"
...
_SINGLE_SKILL_RE = re.compile(
    ...
    r"fastapi|django|flask|sql|mysql|postgres(?:ql)?|mongodb|redis|"
    r"docker|kubernetes|aws|azure|gcp|git|figma|sketch|photoshop|"
    r"illustrator|indesign|excel|powerpoint|word|salesforce|hubspot|"
    r"sap|tableau|powerbi|linux|html|css|php|ruby|swift|kotlin|"
    r"c\+\+|c#|rust|go|scala|r\b|matlab|hadoop|spark|tensorflow|"
    r"pytorch|scikit|pandas|numpy|opencv)\b",
    re.IGNORECASE,
)

_ROLE_TITLE_RE = re.compile(
    r"\b(manager|developer|engineer|designer|analyst|specialist|"
    r"coordinator|supervisor|director|consultant|administrator|"
    r"recruiter|accountant|architect|scientist|researcher)\b",
    re.IGNORECASE,
)
...
def score_applicant(
    ...
    Two modes:
    - Explicit mode: caller passes must_haves (from a structured form).
      Uses w_must_have / w_similarity / w_nice_to_have / w_experience.
    ...
    if explicit_mode:
        ...
        if auto_keywords:
            keyword_ratio, matched_must, missing_must = must_have_coverage(cand_text, auto_keywords)
        else:
