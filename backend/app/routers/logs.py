...
# ── Log categories & their actions ───────────────────────────────────────────
# entity_type: "applicant" | "job" | "employee" | "system"
# action:      free string e.g. "status_changed", "deleted", "created", ...
...
def write_log(
    ...
    """
    Fire-and-forget helper — write one log entry to Firestore.
    Call this from any admin router endpoint; errors are silently swallowed
    so they never break the actual operation.
    """
    try:
        db = get_db()
...
def get_logs(
    ...
    Optional filters: ?entity_type=applicant  ?action=status_changed
    """
    db    = get_db()
    query = db.collection("activity_logs").order_by(
        "timestamp",
